...
static SemaphoreHandle_t i2c_buffer_lock=0;
...
static void i2c_bus_reset()
{
    ...
    gpio_output_options_set(GPIOB, GPIO_OTYPE_OD, GPIO_OSPEED_2MHZ, GPIO_PIN_15);
    gpio_mode_set(GPIOA, GPIO_MODE_AF, GPIO_PUPD_PULLUP, GPIO_PIN_8);
    gpio_output_options_set(GPIOA, GPIO_OTYPE_OD, GPIO_OSPEED_2MHZ, GPIO_PIN_8);
}

/*************************************************************************************************
  * @brief    : init i2c fequency to 400k for default
  * @param1   : null
  * @return   : null
*************************************************************************************************/
void hwi_I2C_Init(uint8_t channel)
{
    if(1 == channel)
    {
        ...
        i2c_timing_config(I2C1, 0, 1, 0);
        i2c_master_clock_config(I2C1, 13, 16);
...
/*************************************************************************************************
  * @brief    : reinit i2c fequency to 100k when fuel gauge in ROM mode
  * @param1   : null
  * @return   : null
*************************************************************************************************/
void hwi_I2C_ReInit(uint8_t channel)
{
    if(1 == channel)
    {
        rcu_periph_clock_enable(RCU_I2C1);
        i2c_deinit(I2C1);
        /* configure I2C timing */
        i2c_timing_config(I2C1, 3, 2, 0);
        i2c_master_clock_config(I2C1, 16, 19);
        /* enable I2C1 */
        i2c_enable(I2C1);
    }
}

void i2c_lock_create(void)
{
    i2c_buffer_lock = xSemaphoreCreateMutex();
}
...
{
    if(xSemaphoreTake(i2c_buffer_lock, 1000)){
        ...
        if(MemAddSize == 1)
        {
            tx_buf[0] = MemAddress & 0xff;
        }
        else if(MemAddSize == 2)
        {
            tx_buf[0] = (MemAddress >> 8) & 0xff;
            tx_buf[1] = MemAddress & 0xff;
        }
        ...
        while(!end_flag){
            ...
            case I2C_START:
                ...
                i2c_transfer_byte_number_config(I2C1, Size+MemAddSize);
                ...
                if(timeout < Timeout){
                    ...
                    /* timeout, bus reset */
                    i2c_bus_reset();
                    xSemaphoreGive(i2c_buffer_lock);
                    return HWI_TIMEOUT;
            ...
            case I2C_SEND_ADDRESS:
                while(MemAddSize--){/* wait until the transmit data buffer is empty */
                while((!i2c_flag_get(I2C1, I2C_FLAG_TBE)) && (timeout < Timeout)){
                    timeout++;
                }
                if(timeout < Timeout){
                    /* send the EEPROM's internal address to write to : only one byte address */
                    i2c_data_transmit(I2C1, tx_buf[count]);
                    count++;
                    timeout = 0;
        ...
        while(!end_flag){
            ...
            case I2C_START:
                if(0 == restart_flag){
                    ...
                    i2c_master_addressing(I2C1, DevAddress<<1, I2C_MASTER_TRANSMIT);
                    /* configure number of bytes to be transferred */
                    i2c_transfer_byte_number_config(I2C1, MemAddSize);
