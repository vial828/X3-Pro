...
static int getRandomNumber(uint64_t *p_vli)
{
    ...
    if(CryptGenRandom(hCryptProv, 16, pbData))
    {
        ...
        memcpy((uint8_t*)p_vli, (uint8_t*)pbData, 16);
    }
    else
    {
...
static int cryalgPoint_isZero(cryalgPoint *p_point)
{
...
static void XYcZ_initial_double(uint64_t *X1, uint64_t *Y1, uint64_t *X2, uint64_t *Y2, uint64_t *p_initialZ)
{
    ...
    cryalgPoint_double_jacobian(X1, Y1, z);
...
{
    uint64_t t5[NUM_cryalg_DIGITS];
    uint64_t t6[NUM_cryalg_DIGITS];
    uint64_t t7[NUM_cryalg_DIGITS];
...
static void cryalgPoint_mult(cryalgPoint *p_result, cryalgPoint *p_point, uint64_t *p_scalar, uint64_t *p_initialZ)
{
    uint64_t Rx[2][NUM_cryalg_DIGITS];
    uint64_t Ry[2][NUM_cryalg_DIGITS];
    uint64_t z[NUM_cryalg_DIGITS];
...
static void cryalg_native2bytes(uint8_t p_bytes[cryalg_BYTES], const uint64_t p_native[NUM_cryalg_DIGITS])
{
    unsigned i;
    for(i=0; i<NUM_cryalg_DIGITS; ++i)
    {
        uint8_t *p_digit = p_bytes + 8 * (NUM_cryalg_DIGITS - 1 - i);
...
{
    uint64_t l_private[NUM_cryalg_DIGITS];
    cryalgPoint l_public;
    unsigned l_tries = 0;
    do{
        ...
        cryalgPoint_mult(&l_public, &curve_G, l_private, NULL);
    } while(cryalgPoint_isZero(&l_public));
    
    cryalg_native2bytes(p_privateKey, l_private);
    cryalg_native2bytes(p_publicKey + 1, l_public.x);
...
int cryalgh_shared_secret(const uint8_t p_publicKey[cryalg_BYTES+1], const uint8_t p_privateKey[cryalg_BYTES], uint8_t p_secret[cryalg_BYTES])
{
    cryalgPoint l_public;
    uint64_t l_private[NUM_cryalg_DIGITS];
    uint64_t l_random[NUM_cryalg_DIGITS];
    ...
    cryalg_bytes2native(l_private, p_privateKey);
    
    cryalgPoint l_product;
    cryalgPoint_mult(&l_product, &l_public, l_private, l_random);
    
    cryalg_native2bytes(p_secret, l_product.x);
    
    return !cryalgPoint_isZero(&l_product);
}
...
{
    uint64_t k[NUM_cryalg_DIGITS];
    uint64_t l_tmp[NUM_cryalg_DIGITS];
    uint64_t l_s[NUM_cryalg_DIGITS];
    cryalgPoint p;
    unsigned l_tries = 0;
    ...
    do{
        ...
        cryalgPoint_mult(&p, &curve_G, k, NULL);
        if(vli_cmp(curve_n, p.x) != 1){
            vli_sub(p.x, p.x, curve_n);
        }
    } while(vli_isZero(p.x));

    cryalg_native2bytes(p_signature, p.x);
    
    cryalg_bytes2native(l_tmp, p_privateKey);
    vli_modMult(l_s, p.x, l_tmp, curve_n); 
    cryalg_bytes2native(l_tmp, p_hash);
    vli_modAdd(l_s, l_tmp, l_s, curve_n); 
    vli_modInv(k, k, curve_n); 
    vli_modMult(l_s, l_s, k, curve_n); 
    cryalg_native2bytes(p_signature + cryalg_BYTES, l_s);
